...
from commands import parse_command, handle_command, CommandContext, _print_recent_conversation_preview
...
def _maybe_auto_compact(engine: Engine, compact_service: CompactService,
    ...
    try:
        ...
        console.print(
            f"[dim]Context compressed: {len(messages)} → {len(new_msgs)} messages, "
...
def main() -> None:
    ...
    if args.resume and session_store is not None:
        ...
        if target:
            ...
            if msgs:
                ...
                _print_recent_conversation_preview(console, msgs, rounds=2)
        else:
    ...
    if args.print or args.prompt:
        ...
        run_query(
            ...
            turn_context={"source": "non_interactive", "command": "print" if args.print else "prompt"},
        )
    ...
    while True:
        ...
        try:
            ...
            user_input = bordered_prompt(   # 阻塞等待用户输入
                console,
        ...
        compact_info = _maybe_auto_compact(
            engine,
            compact_service,
            cost_tracker,
            app_config.model,
