...
def _recent_conversation_messages(messages: list[dict], rounds: int = 2) -> list[tuple[str, str]]:
    ...
    for message in messages:
        ...
        if not text:
            continue
        visible.append((role, " ".join(text.split())))
    keep = max(1, rounds) * 2
    return visible[-keep:]


def _print_recent_conversation_preview(console: Console, messages: list[dict], rounds: int = 2) -> None:
    preview = _recent_conversation_messages(messages, rounds=rounds)
    if not preview:
        return
    shown_rounds = min(rounds, max(1, (len(preview) + 1) // 2))
    console.print(f"[dim]Recent conversation from resumed session ({shown_rounds} rounds):[/dim]")
    for role, text in preview:
        label = "You" if role == "user" else "Assistant"
        snippet = text if len(text) <= 200 else text[:197] + "..."
        console.print(f"[dim]{label}:[/dim] {snippet}", highlight=False)
...
def _cmd_resume(ctx: CommandContext, args: str) -> None:
    ...
    _print_recent_conversation_preview(ctx.console, messages, rounds=2)
    ...
    if skill.context == "fork":
        ...
        try:
            ...
            run_query(
                ...
                turn_context={"source": "skill", "skill": skill.name, "skill_context": "fork"},
            )
        finally:
        ...
        run_query(
            ...
            print_mode=False,
            permissions=permissions,
            turn_context={"source": "skill", "skill": skill.name, "skill_context": "inline"},
        )
