...
def default_model_for_provider(provider: str) -> str:   # 默认模型
...
def default_companion_model(provider: str, model: str) -> str:  # 提供商返回默认的模型
    provider = validate_provider(provider)
    if provider == _OPENAI_PROVIDER:
        return model
...
class LLMClient:
    def __init__(
        ...
        if self.provider == _OPENAI_PROVIDER:
            if OpenAI is None:
                message = "OpenAI 模型支持需要 'openai' 模块。"
                if _OPENAI_IMPORT_ERROR is not None:
                    message += f" OpenAI 模块导入失败: {_OPENAI_IMPORT_ERROR}"
    ...
    def is_authentication_error(self, exc: Exception) -> bool: # 是否为身份验证错误
        if self.provider == _OPENAI_PROVIDER:
    ...
    def is_retryable_error(self, exc: Exception) -> bool:   # 判断给定的异常是否为可重试的错误
    ...
    def is_api_error(self, exc: Exception) -> bool: # 是否为API错误
        if self.provider == _OPENAI_PROVIDER:
            return openai is not None and isinstance(exc, openai.APIError)
        return isinstance(exc, anthropic.APIError)

    @staticmethod
    def error_message(exc: Exception) -> str:   # 将异常对象转换为错误信息字符串
...
#   该方法用于调用OpenAI API创建消息，并返回处理后的LLMMessage对象
...
class _OpenAIStream:
    ...
    def _iter_text(self) -> Iterator[str]:  # 迭代处理流式文本响应
...
def _normalize_openai_stop_reason(reason: str | None) -> str | None:    # 将 OpenAI 的 finish_reason 映射为 Anthropic 的 stop_reason 等效值
...
# 规范化 Anthropic 格式的内容，将其转换为内部消息格式的列表
...
# 规范化 Anthropic API 的消息块，将不同类型的块转换为统一的字典格式
...
# 规范化 OpenAI API 的消息内容，将其转换为内部消息格式的列表
...
# 将系统消息和消息列表转换为OpenAI API兼容的消息格式。
'''
内部消息格式：
{
  "role": "user",
...
[
  {"type": "text", "text": "你好世界"}
  {
    "type": "image_url",
    "image_url": {
      "url": "data:image/png;base64,some_base64_data_here"
    }
  }
]
'''
