...
# "你的回复应当简短精炼。",
# "引用特定函数或代码片段时，请包含 file_path:line_number（文件路径:行号）格式，以便用户轻松导航至源代码位置。",
# "引用 GitHub issues 或 pull requests 时，请使用 owner/repo#123 格式（例如：anthropics/claude-code#100），以便它们渲染为可点击的链接。",
# "不要在工具调用前使用冒号。你的工具调用可能不会直接显示在输出中，因此像 'Let me read the file:' 后跟读取工具调用的文本，应改为 'Let me read the file.'（以句号结尾）。"
...
# 调用子进程获取当前工作目录、操作系统类型（如 Windows）、默认终端环境
...
# 注入Git 的当前分支、状态（status）和最近 5 条 commit 记录。
...
# 当处于计划模式时，注入多阶段规划工作流（初始探索、设计、审查、最终计划），并强制限制该阶段仅能使用只读工具及 Explore 类型子 Agent。
